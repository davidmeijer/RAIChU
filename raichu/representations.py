...
class ClusterRepresentation:
            ...
            for module_idx, module in enumerate(self.modules):
                for domain in module.domains:
                    out.write(
                        ...
                        f"\t{domain.used}"
                        f"\t{domain.active}"
                        f"\t{domain.translation}"
                        "\n"
                    )

    def write_cluster_tailoring(self, out_path: str) -> None:
        """Write cluster tailoring elements to a file."""
        with open(out_path, "w") as tailoring:
            tailoring.write(
                "gene_name"
                "\ttype"
                "\tsubstrate"
    ...
    def from_cluster_files(cls, in_cluster, in_tailoring):
        ...
        if in_tailoring and os.path.exists(in_tailoring):
            tailoring_enzymes = []
            with open(in_tailoring, "r") as tailoring:
                tailoring.readline()
                for line in tailoring:
